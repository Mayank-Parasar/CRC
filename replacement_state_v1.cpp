...
void CACHE_REPLACEMENT_STATE::InitReplacementState()
{
    ...
    {
        repl[ setIndex ]  = new LINE_REPLACEMENT_STATE[ assoc ]; //repl is a 2-dimentional array [setIndex][wayIndex]
    ...
    {
        // RRIP always inserts new block with long re-refernce interval
        // Create the state for the sets
        for(UINT32 setIndex=0; setIndex<numsets; setIndex++) 
        {
            repl[ setIndex ]  = new LINE_REPLACEMENT_STATE[ assoc ]; //repl is a 2-dimentional array [setIndex][wayIndex]
            ...
            for(UINT32 way=0; way<assoc; way++) 
            {
                // Initially we assume all the lines have re-reference in distant future.
                repl[ setIndex ][ way ].RRPV = (1 << (M)) - 1; // 2^M -1
            }
        }    
    }
...
INT32 CACHE_REPLACEMENT_STATE::Get_SRRIP_Victim( UINT32 setIndex )
{
    ...
    UINT32 srripWay = 0;
    // Search for victim whose RRVP value is 2^M-1
    // If couldn't find then incrememnt each way RRVP by 1 and
    // search again
    // label
    startOver:
    UINT32 way = 0;
    ...
    {
        if (replSet[way].RRPV == (unsigned int)((1<<M) - 1))
                {
                    // you have potentially found the victim
                    // return the victim
                    srripWay = way;
                    return srripWay;
                }        
...
{
    // printf("Control coming to Update SRRIP!!!\n");
    // Set the 'RRVP' of the way to 0 on hit
    if (cacheHit)
    {
